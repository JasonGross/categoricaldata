  ...
  def renderView(request: Request, response: Response, models: Seq[Any]) = {
    ...
    prettyJsonViewRenderer.renderView(request, response,
      models collect {
        ...
        case dataset: net.metaphor.api.Ontology#Dataset => net.metaphor.json.Pack.packDataset(dataset)
        case translation: net.metaphor.api.Translation => net.metaphor.json.Pack.packTranslation(translation)
      })
  }
}