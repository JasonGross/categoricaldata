...
trait FinitelyPresentedCategory extends FinitelyGeneratedCategory { fpCategory =>
  ...
  protected trait Wrapper extends super.Wrapper with FinitelyPresentedCategory {
    override def relations(s: O, t: O) = fpCategory.relations(s, t)
  }

  abstract protected class FullSubcategory(spannedBy: List[O]) extends super.FullSubcategory(spannedBy) with FinitelyPresentedCategory {
    override def relations(s: O, t: O) = fpCategory.relations(s, t)
  }
  ...
  trait FunctorToSet extends super.FunctorToSet with Functor.withFinitelyPresentedSource { functorToSet =>
    //    Commenting out the following line, things still compile, but we get AbstractMethodError everywhere:
    override val source: fpCategory.type = fpCategory
